...
def run_timeloop_mapper(arch, prob, mapspace, cwd=os.getcwd(), stdout=None, stderr=None):
    try:
        print(cwd)
        print('run_timeloop> /scratch/qijing.huang/timeloop-new/build/timeloop-mapper {} {} {}'.format(arch, mapspace, prob))
        p = subprocess.check_call(['/scratch/qijing.huang/timeloop-new/build/timeloop-mapper', arch, mapspace, str(prob), ], \
                                  cwd=cwd, stdout=stdout, stderr=stderr)
