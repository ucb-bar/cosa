    ...
    try:
        ...
        if nb: 
            p = subprocess.check_call(['./' + exe, str(prefix)],\
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False, cwd=os.getcwd(),  timeout=timeout)
        else: 
            p = subprocess.check_call(['./' + exe, str(prefix)],\
...
def get_cor_stats(status_dicts):
    ...
    for d in status_dicts:
        ...
        l_arr.append( (d['utilized_capacity'][1] + d['utilized_capacity'][2] + d['utilized_capacity'][3] + d['utilized_capacity'][4]) )

        l_arr.append( (d['utilized_capacity'][1] * d['utilized_capacity'][2] * d['utilized_capacity'][3]) ** (1/128))
        l_arr.append( (d['utilized_capacity'][1] * d['utilized_capacity'][2] * d['utilized_capacity'][3] * d['utilized_capacity'][4]) ** (1/128))
        l_arr.append( (np.log2(1+d['utilized_capacity'][1]) + np.log2(1+d['utilized_capacity'][2]) + np.log2(1+d['utilized_capacity'][3])))
        l_arr.append( (np.log2(1+d['utilized_capacity'][1]) + np.log2(1+d['utilized_capacity'][2]) + np.log2(1+d['utilized_capacity'][3]) + np.log2(1+d['utilized_capacity'][4])))
