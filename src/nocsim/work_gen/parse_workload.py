...
var_dep = {
    ...
    "Outputs_Update" : ["Weights", "Inputs", "Outputs"] # Must wait until last output is written to the buf? maybe delete Outputs 
        }
var_bits = {
    "Weights" : 8,
    "Inputs" : 8,
...
For Outputs, True indicates multicast, False indicates reduction.):\n\t{}""".format(buf_config))
    ...
    logger.info("Iter Start Dimension (Loop dimension the variable starts vary, inner to outer):\n\t{}".format(iter_start_dim))
    ...
    for i in range(output_start_dim, len(temporal_loops)):
        ...
        if dimension in partial_sum_dep_prob: 
            ...
            break
    logger.info("Outputs Start Dimension {}, Partial Sum Depend Dimension:{}".format(output_start_dim, partial_sum_dep_prob_idx))


#    num_steps = 0
#    steps = {}
#    reset_var_dict_list(steps)
#    steps["Outputs_Store"] = list()
#     
#    logger.info("Transaction at Each Iteration:")
#    while(not is_done(temporal_loops, temporal_loop_iter)):
#        print_loop_indices(temporal_loop_iter)
#
#        # init new entries with 1, 1 indicates traffic in that cycle 
#        send_var_names = ['Weights', 'Inputs']
#        for var in all_var_names:
#            if var == 'Outputs':
#                if partial_sum_dep_prob_idx == len(temporal_loops):
#                    steps[var].append(0)
#                else:
#                    steps[var].append(1)
#            else:
...
#
#        #steps['Offsets'] = 0
#        steps["Outputs_Store"].append(1) 
#
#        # for all var, check if it is time to send to PE
#        # read Outputs partial sums
#        for var in all_var_names: 
#        #for var in send_var_names: 
#            # assume start_dim is 2, the 0th and 1st lowest idx of
#            # temporal_loop_iter is must be 0
#            # first update would be (0,0,0), second will be (0,0,1)
#            start_dim = iter_start_dim[var]
#
#            # no traffic is in cur inner loop iter 
#            for i, it in enumerate(temporal_loop_iter[0:start_dim]):
#                # set to no traffic if current iter is not the begin of the dim
#                if it != temporal_loops[i]['start']:
#                    steps[var][-1] = 0
#
#            if var == 'Outputs' and partial_sum_dep_prob_idx != len(temporal_loops): 
#                if temporal_loop_iter[partial_sum_dep_prob_idx] == temporal_loops[partial_sum_dep_prob_idx]['start']: 
#                    steps[var][-1] = 0
#
#            if steps[var][-1] == 1:
#                logger.info("\t\tRead {0} from DRAM".format(var))
#
#
#        # for outputs, check if it is time to writeback to DRAM 
#        start_dim = iter_start_dim['Outputs'] 
#        if start_dim > 0:
#            # for temperal loops of bounds 2,3,4,
#            # start_dim = 2, 
#            # updates at 
#            # (1,2,0), (1,2,1), (1,2,2), (1,2,3)
#            for i, it in enumerate(temporal_loop_iter[0:start_dim]):
#                if it != temporal_loops[i]['end'] - 1:
#                    steps['Outputs_Store'][-1] = 0
#
#            # get the inner loop bound
#            #cur_inner_loop_idx = temporal_loop_iter[start_dim-1] 
#            #inner_loop_stride = temporal_loops[start_dim-1]['stride']
#            #inner_loop_end = temporal_loops[start_dim-1]['end']
#            #if cur_inner_loop_idx + inner_loop_stride < inner_loop_end:
#            #    steps['Outputs_Store'][-1] = 0
#        # if start_dim == 0, the default is 1 
#
#        if steps['Outputs_Store'][-1] == 1:
#            logger.info("\t\tStore Outputs from AccumulationBuffer to DRAM")
#
#        increment(temporal_loops, temporal_loop_iter)
#        num_steps += 1
#
...
def get_cost(iters, data_size, buf_spatial):
    ...
    for var in ['Weights', 'Inputs', 'Outputs']:
        ...
        logger.info("\t\t{}: iter={}, data_size={}, var_bits={}, unicast_count={}, cost={}".format(var, iters[var], data_size[var], var_bits[var], len(addr), cost[var]))
        total_cost += cost[var]
    ...
    logger.info("\t {}: iter={}, data_size={}, var_bits={}, unicast_count={}, cost={}".format(var, iters[var], data_size['Outputs'], var_bits['Outputs'], len(addr), cost[var]))
    total_cost += cost[var]
